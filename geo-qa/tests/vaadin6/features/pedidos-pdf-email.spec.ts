...
test.describe('Pedidos Clientes Pendentes - PDF Email', () => {
  test.beforeEach(async ({ page }) => {
    await loginIGS(page);
  });

  test('envia pedido por email @e2e', async ({ page }) => {
    ...
    // Enviar por E-mail
    await page.locator("//span[contains(text(),'Enviar por E-mail')]").click();

    // Verificar que a tela de envio esta presente
    const envioExists = await verificaExistencia(page, "(//div[contains(text(),'col1')])[1]");
    expect(envioExists).toBeTruthy();
